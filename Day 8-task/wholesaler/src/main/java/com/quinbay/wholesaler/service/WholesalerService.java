...
public class WholesalerService implements WholesalerInterface {
    ...
    @Override
    public ResponseEntity<Wholesaler> findWholesaler(int wid) {
        Optional<Wholesaler> opt = wholesalerRepository.findById(wid);
    ...
    public String addWholesaler(ArrayList<Wholesaler> inputWholesaler) {
        for (Wholesaler w : inputWholesaler) {
            Wholesaler wholesaler = new Wholesaler(w.getId(), w.getWhcode(), w.getWhname());
    ...
    public Map<String, Boolean> deleteWholesalerById(int id) {
        ...
        @Override
        public String updateWholesaler ( int id, int code)
        {
            Optional<Wholesaler> opt = wholesalerRepository.findById(id);
            if (opt.isPresent()) {
                opt.get().setWhcode(code);
                wholesalerRepository.save(opt.get());
                return "Wholesaler data updated successfully";
            }
            return "Wholesaler id doesn't exist";
        }
    }
